...
export default function ResultsCarriersSection({
  ...
  return (
    <section className="relative overflow-hidden bg-[#161616] px-4 py-14 text-white sm:px-8 sm:py-16 lg:px-12 lg:py-20">
      ...
      <div className="relative z-10 mx-auto grid w-full max-w-[97.5rem] gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(25rem,37.5rem)] lg:items-center xl:gap-16">
        <div className="relative min-h-[24rem] sm:min-h-[30rem] lg:min-h-[34rem]">
          <Image
            src="/MC DOT Setup/icons/4_vector_3.svg"
            alt=""
            fill
            sizes="(max-width: 63.9375rem) 100vw, 50vw"
            className="object-contain object-left-center opacity-60"
            unoptimized
          />
          <Image
            src={image}
            alt={imageAlt}
            fill
            sizes="(max-width: 63.9375rem) 100vw, 48rem"
          ...
          <div className="relative mt-8 max-h-[28rem] overflow-y-auto pr-5 [scrollbar-color:#b34b0c_#3f3f3f] [scrollbar-width:thin]">
            <div className="space-y-4">
              {steps.map((step, index) => (
                <article key={step.title} className="grid grid-cols-[3.5rem_minmax(0,1fr)] gap-4 border border-[#313131] bg-[#1c1c1c] p-4 sm:grid-cols-[4.5rem_minmax(0,1fr)] sm:p-5">
                  ...
                  <div className="min-w-0">
                      ...
                      <>
                        ...
                        <ul className="mt-2 grid gap-1 sm:grid-cols-2">
                          {step.includes.map((item) => (
                            <li key={item} className="flex items-start gap-2 font-['DM_Sans'] text-[0.6875rem] leading-[1.35] text-white/75 sm:text-xs">
                    ...
                    {step.footer ? (
                      <p className="mt-3 font-['DM_Sans'] text-xs leading-[1.4] text-white/80 sm:text-sm">{step.footer}</p>
                    ) : null}
                  </div>
                </article>
              ))}
...
function Wave({ className }: { className: string }) {
  return (
    <svg className={`absolute left-0 h-8 w-full ${className}`} viewBox="0 0 1440 32" preserveAspectRatio="none" aria-hidden="true">
