...
function Logo() {
  return (
    ...
    >
      <Image
        src="/Home/images/2_rectangle_8.webp"
        alt=""
        width={58}
        height={42}
      ...
      <span className="text-[1.375rem] font-black italic leading-none tracking-[-0.04em]">
        DISPATCHING
      </span>
    </Link>
  );
    ...
    <footer className="bg-[#161616] text-[#d6d6d6]">
      <div className="mx-auto grid w-full max-w-[69.1875rem] gap-10 px-4 pb-10 pt-8 min-[23.4375rem]:px-6 sm:px-10 md:grid-cols-2 lg:grid-cols-[repeat(4,minmax(0,1fr))] lg:gap-x-10 lg:gap-y-0 lg:px-[4.4375rem] lg:pb-5 lg:pt-[2rem] xl:gap-x-12">
        <div>
          ...
          <p className="mt-4 max-w-[17rem] text-[0.8125rem] leading-[1.3rem] text-[#d1d1d1]">
            MNA Transport offers reliable and profit driven truck dispatching
            services across the United States. We help owner operators and
            fleets secure high-paying loads, reduce deadhead miles, and
            streamline operations for maximum efficiency.
        ...
        <div>
          <h2 className="mb-4 text-[1rem] font-bold text-[#df5708]">
            Get In Touch
          </h2>

          <div className="space-y-4 text-[0.875rem] leading-[1.35rem] text-[#e0e0e0]">
            <div>
              <ContactLabel icon="phone" text="Phone" />
      ...
      <div className="border-t border-white/10">
        <div className="mx-auto flex min-h-[3.75rem] w-full max-w-[69.1875rem] flex-col items-center justify-between gap-3 px-4 py-4 text-center text-[0.75rem] leading-5 min-[23.4375rem]:px-6 sm:px-10 md:flex-row md:text-left lg:px-[4.4375rem]">
          <p>
            © 2026, MNA Transport, Design &amp; Developed By{" "}
            <span className="font-bold text-white">BitBlazeTec</span>
          </p>
...
function FooterLinks({
  ...
  return (
    <div>
      ...
      <ul className="space-y-2 text-[0.875rem] leading-[1.35rem]">
        {links.map(([label, href]) => (
          <li key={label}>
            <Link
              href={href}
...
function ContactLabel({
  ...
  return (
    <span className="mb-1.5 flex items-center gap-2 text-[0.75rem] font-semibold text-white">
      <svg
        viewBox="0 0 18 18"
        aria-hidden="true"
        className="h-4 w-4 shrink-0 fill-none stroke-[#df5708] stroke-[1.2]"
