...
export default function ServiceGuaranteesSection({
  ...
  return (
    <section
      className={`bg-[#181818] px-5 py-12 text-white sm:px-8 lg:px-12 lg:py-[3.125rem] ${className}`}
    >
      <div
      ...
      >
        ...
        <div className="flex min-w-0 flex-col">
          ...
          <div
            className={`mt-8 grid flex-1 auto-rows-fr ${
              isProcessVariant
                ? "gap-4 lg:mt-9 lg:gap-[0.9375rem]"
                : "gap-4 lg:mt-9 lg:gap-[0.9375rem]"
            }`}
          >
            {cards.map((card, index) => (
              <article
                key={index}
                className="flex min-h-[9.5rem] flex-col justify-center border border-[#404040] bg-zinc-900 px-6 py-6 sm:px-[1.875rem] lg:min-h-0 lg:py-5"
              >
