...
export default function EquipmentShowcaseSection({
  ...
  return <section className="pb-16 pt-16">
      <div className="mx-auto w-[calc(100%-40px)] max-w-[1560px] sm:w-[calc(100%-64px)]">
        ...
        <div className="mt-20 grid gap-x-[11px] gap-y-16 sm:grid-cols-2 xl:grid-cols-4">
          {columns.map((column, columnIndex) => <div key={columnIndex} className="space-y-[72px]">
            
              {column.map(equipmentIndex => {
            const name = names[equipmentIndex];
            const reverse = columnIndex % 2 === 1;
            const image = <div className="relative w-full" style={{
              height: imageHeights[equipmentIndex]
            }}>


                
                    <Image src={images[equipmentIndex]} alt={`${name} dispatch`} fill sizes="(min-width: 1280px) 384px, (min-width: 640px) 50vw, 100vw" className="object-contain" />
                
                  </div>;
            const copy = <div className="flex min-h-[17rem] flex-col px-12 py-6">
                    ...
                    <div className="mt-auto inline-flex items-center gap-3 pt-7 font-['DM_Sans'] text-sm font-medium text-[#b34b0c]">
                      ...
                      <span className="h-0.5 w-20 origin-left bg-[#b34b0c]   group/equipment-item:hover:scale-x-125" />
                    </div>
                  </div>;
            return <article key={name} className="group/equipment-item">
