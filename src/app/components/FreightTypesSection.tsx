...
export default function FreightTypesSection({
  ...
  const gridColumns = resolvedColumns === 4 ? "xl:grid-cols-4" : "xl:grid-cols-3";
  return <section className="bg-[#1c1c1c] px-5 py-16 text-white sm:px-8 lg:py-20">
      <div className="mx-auto max-w-[97.5rem]">
        <div className="mx-auto max-w-[46rem] text-center">
          <h2 className="font-outfit text-3xl font-bold leading-[1.15] sm:text-4xl lg:text-[2.75rem]">
            {heading}
        ...
        <div className={`mt-12 grid gap-5 md:grid-cols-2 ${gridColumns}`}>
          {freightTypes.map(freightType => {
          const iconPath = freightType.iconSrc || freightType.imageSrc || freightType.logoSrc || "";
          const iconAlt = freightType.iconAlt || freightType.imageAlt || freightType.logoAlt || freightType.title;
          return <article key={freightType.title} className={`flex flex-col bg-[#161616] ${isCompact ? "min-h-[17.125rem] items-center justify-center px-6 py-7 text-center" : "min-h-[27rem] p-6 text-left sm:p-7 lg:p-8"}`}>
                <span className="flex h-14 w-14 shrink-0 items-center justify-center bg-[#b34b0c]/20 text-[#b34b0c]">
                  {iconPath ? <Image src={iconPath} alt={iconAlt} width={32} height={32} className="h-8 w-8 object-contain" /> : freightType.icon ?? <FreightTruckIcon />}
                </span>

                <h3 className={`font-outfit text-xl font-semibold leading-tight ${isCompact ? "mt-6" : "mt-7"}`}>
                  {freightType.title}
                </h3>

                <p className={`font-dm-sans text-sm leading-6 text-white/85 ${isCompact ? "mt-4 max-w-[19rem]" : "mt-6"}`}>
                  {freightType.description}
                </p>

                {!isCompact && freightType.items?.length ? <ul className="mt-5 space-y-2 text-left">
                    {freightType.items.map(item => <li key={item} className="flex gap-3 font-dm-sans text-sm leading-5 text-white/85">
                        <span className="mt-[0.45rem] h-1.5 w-1.5 shrink-0 rounded-full bg-[#b34b0c]" />
                        {item}
                      </li>)}
                  </ul> : null}

                {!isCompact && freightType.footer ? <p className="mt-auto pt-6 font-dm-sans text-sm leading-6 text-white/85">
                    {freightType.footer}
                  </p> : null}
              </article>;
        })}
...
function FreightTruckIcon() {
  return <svg aria-hidden="true" viewBox="0 0 32 24" className="h-7 w-8 fill-none stroke-current" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
      <path d="M2 4h18v13H2zM20 9h5l5 5v3H20z" />
      <path d="M5 7h12M5 10h12M7 21a3 3 0 1 0 0-6 3 3 0 0 0 0 6ZM25 21a3 3 0 1 0 0-6 3 3 0 0 0 0 6Z" />
    </svg>;
}
