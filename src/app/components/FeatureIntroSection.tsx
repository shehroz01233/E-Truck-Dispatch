...
export default function FeatureIntroSection({
  ...
  const descriptions = Array.isArray(description)
    ...
    : description
      ? [description]
      : [];
  const footerDescriptions = Array.isArray(footerDescription)
    ? footerDescription
    : footerDescription
      ? [footerDescription]
      : [];
  ...
  if (variant === "cards") {
    return (
      <section className="mx-auto w-[calc(100%-32px)] max-w-[1560px] py-12 text-white sm:w-[calc(100%-48px)] sm:py-16 lg:w-[calc(100%-64px)] lg:py-20">
        <div className="mx-auto max-w-[48rem] text-center">
          <h2 className="font-['Outfit'] text-3xl font-bold leading-tight sm:text-4xl md:text-5xl">
            {heading}
          </h2>
          {descriptions.length > 0 && (
            <div className="mx-auto mt-6 max-w-[35rem] space-y-4 font-['DM_Sans'] text-sm font-medium leading-6 text-white/75 sm:text-base">
        ...
        <div className="mt-12 grid gap-6 md:grid-cols-3">
          {cards.map((card) => (
            <article key={card.title} className="bg-[#171717] p-6 sm:p-8">
              <span className="flex h-12 w-12 items-center justify-center bg-[#b34b0c]/20 text-[#b34b0c]">
                {card.iconSrc ? (
                  ...
                  <span className="[&_svg]:h-8 [&_svg]:w-8">{card.icon}</span>
                ) : (
                  <span className="h-2 w-2 rounded-full bg-[#b34b0c]" />
                )}
              </span>
      ...
      <section className="mx-auto w-[calc(100%-32px)] max-w-[1560px] py-12 text-white sm:w-[calc(100%-48px)] sm:py-16 lg:w-[calc(100%-64px)] lg:py-20">
        <div className="relative overflow-hidden bg-[#161616] px-6 py-10 sm:px-10 lg:min-h-[39rem] lg:px-12 lg:py-14">
          <div className="relative z-10 max-w-[52rem]">
            ...
            {descriptions.length > 0 && (
              <div className="mt-6 max-w-[50rem] space-y-4 font-['DM_Sans'] text-sm font-medium leading-6 text-white/80 sm:text-base">
                {descriptions.map((paragraph, index) => (
                  <p key={index}>{paragraph}</p>
                ))}
              </div>
          ...
          <div className="relative z-10 mt-10 grid gap-5 md:grid-cols-3 lg:max-w-[58rem]">
            {cards.map((card) => (
              <article
                key={card.title}
                className="border border-white/15 bg-[#161616]/90 p-6"
              >
          ...
          {image && (
            <div className="relative mt-10 min-h-[18rem] lg:absolute lg:bottom-12 lg:right-0 lg:mt-0 lg:h-[33rem] lg:w-[45rem]">
      ...
      <div className="grid items-center gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(380px,719px)] lg:gap-12 xl:gap-14">
        <div className="min-w-0">
          ...
          {footerDescriptions.length > 0 && (
            <div className="mt-6 max-w-[649px] space-y-4 font-['DM_Sans'] text-base font-medium leading-7 text-white/80">
              {footerDescriptions.map((paragraph, index) => (
                <p key={index}>{paragraph}</p>
              ))}
            </div>
          )}
