...
export default function FAQSection({
  ...
  return <section className="overflow-hidden bg-[#1c1c1c] px-[1rem] py-[3rem] text-white min-[30rem]:px-[1.5rem] min-[48rem]:px-[2rem] min-[64rem]:px-[3rem] min-[80rem]:py-[5rem]">
      <div className="mx-auto grid w-full max-w-[97.5rem] gap-[2.5rem] min-[80rem]:grid-cols-[minmax(0,50rem)_minmax(0,44.375rem)] min-[80rem]:items-start min-[80rem]:gap-[3.125rem]">
        <div className="order-1 flex min-h-0 w-full min-w-0 flex-col overflow-hidden min-[80rem]:order-2">
          ...
          <div className="relative mt-[2rem] aspect-[710/304] w-full shrink-0 overflow-hidden">
            <Image src={image} alt={imageAlt} fill sizes="(max-width: 47.9375rem) 100vw, (max-width: 79.9375rem) 90vw, 44.375rem" className="object-cover" />
        ...
        <div className="order-2 space-y-[0.875rem] min-[80rem]:order-1">
          ...
          return <div key={faq.question} className="overflow-hidden">
                <div className={`relative flex min-h-[3.5rem] w-full items-center justify-between gap-[1rem] overflow-hidden px-[1.25rem] py-[1rem] text-left  min-[30rem]:min-h-[3.75rem] min-[30rem]:px-[1.5rem] min-[64rem]:min-h-[4rem] min-[64rem]:px-[2rem] min-[80rem]:min-h-[4.25rem] min-[80rem]:px-[2.5rem] ${isActive ? "bg-[#b34b0c]" : "bg-[#161616]"}`}>
                  
                  <span aria-hidden="true" className="pointer-events-none absolute bottom-0 left-0 h-px bg-white/35" />
                  ...
                  <button type="button" aria-label={faq.question} aria-expanded={isActive} aria-controls={answerId} onClick={() => setActiveIndex(isActive ? -1 : index)} className="absolute inset-0 z-10 cursor-pointer bg-transparent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-white/80" />
                  
                </div>

                <>
                  {isActive ? <div id={answerId} className="overflow-hidden">
...
function Chevron({
  ...
  return <svg aria-hidden="true" viewBox="0 0 20 20" className={`h-[1.25rem] w-[1.25rem] shrink-0 fill-none stroke-current stroke-2 ${open ? "rotate-180" : ""}`}>
